...
function actionLabel(value: string) {
  return ({ create: '创建', update: '更新', delete: '删除', confirm: '确认异常', revise: '版本修订', submit: '提交', approve: '批准', revoke: '撤销', schedule: '安排执行', complete: '完成反馈', execute: '计划执行', restrict: '建立停喂', dispose: '提交处置', release: '解除停喂' } as Record<string, string>)[value] || value
}
    ...
    <section class="workspace-panel">
      <div class="panel-toolbar">
        <div class="filters audit-filters">
          ...
          <el-select v-model="params.action" placeholder="全部操作" clearable><el-option label="创建" value="create" /><el-option label="更新" value="update" /><el-option label="批准" value="approve" /><el-option label="撤销" value="revoke" /><el-option label="完成反馈" value="complete" /><el-option label="删除" value="delete" /><el-option label="建立停喂" value="restrict" /><el-option label="提交处置" value="dispose" /><el-option label="解除停喂" value="release" /></el-select>
        </div>
      </div>
